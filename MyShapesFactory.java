/*  Drawing random lines, rectangles and ovals
 */
...
public class MyShapesFactory { 
   ...
   public List<MyShape> getRandomMyShapesList(final int NUMBER_OF_SHAPES) {
      MyShape[] myShapesArray = getRandomMyShapes(NUMBER_OF_SHAPES);
      List<MyShape> myShapesList = Arrays.asList(myShapesArray);
      
      return myShapesList;
   }
   
   public Collection<MyShape> getRandomMyShapesCollection(final int NUMBER_OF_SHAPES) {
      List<MyShape>       myShapesList = getRandomMyShapesList(NUMBER_OF_SHAPES);
      Collection<MyShape> collection   = Collections.unmodifiableCollection​(myShapesList);
   ...
   private MyBoundedShape createMyShape(int number, Color strokeColor) {
      Color filledColor = getRandomColor();
      boolean filled    = isFilled();
      
      switch (number) {
         case 1:
            MyBoundedShape rectangle = new MyRectangle(COORDINATIONS[0], COORDINATIONS[1], COORDINATIONS[2], COORDINATIONS[3],
                           strokeColor, filledColor, filled);
            return rectangle;
         case 2:
            MyBoundedShape oval = new MyOval(COORDINATIONS[0], COORDINATIONS[1], COORDINATIONS[2], COORDINATIONS[3],
                           strokeColor, filledColor, filled);
            return oval;
         default:
